...
mod miri_test {
    ...
    fn leak() {
        ...
        let mut frozen_map: FrozenMap<u32, String> = FrozenMap::from_vec(keys.clone());

        // upsert(k, v) ~ replace value if the key exists adn is not reaped
        let _ = frozen_map.upsert(1, "gamma".to_string());
        let _ = frozen_map.upsert(2, "gadmma".to_string());
        let _ = frozen_map.upsert(3, "gammaa".to_string());
        let _ = frozen_map.upsert(4, "g".to_string());
